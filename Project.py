...
with open("description.txt", "r", encoding="utf-8") as description_file, \
        open("members.txt", "r", encoding="utf-8") as members_file:
    ...
    for num in range(6):
        ...
        for line in description_file:
            current_project_description.append(line.strip(f"#{chr(ord("A")+current_project_number)})"))
            if line.startswith("#") :
    ...
    for num in range(6):
        ...
        for line in members_file:
            current_project_members.append(line.strip(f"#{chr(ord("A")+current_project_number)})"))
            if line.startswith("#"): 
...
def open_menu2(previous_window):
    ...
    project6 = tb.Button(menu2, text="Project 6", bootstyle="primary,outline",command=project_6)
...
def open_project_details(project_number, previous_window):
    ...
    # Adding image
    global image
    image = PhotoImage(file=f"photo{project_number}.png")
